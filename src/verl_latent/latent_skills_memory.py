...
class SkillsOnlyMemory(BaseMemory):
    ...
    * ``"embedding"`` – the task description is encoded with a
    ...
    def format_for_prompt(self, retrieved_memories: Dict[str, Any]) -> str:
        """
        Format retrieved skills as latent skill tokens for prompt injection.

        MODIFIED from SkillRL original: instead of outputting text skill descriptions,
        outputs special token strings (e.g., SKILL_gen_001_a SKILL_gen_001_b) that the
        tokenizer will map to pre-initialized latent skill embeddings.

        Args:
            retrieved_memories: Dict returned by :meth:`retrieve`.

        Returns:
            String of skill token names to insert into the agent prompt.
        """
        skill_tokens = []
        ...
        for skill in retrieved_memories.get('general_skills', []):
            ...
            skill_tokens.append(f"SKILL_{skill_id}_a")
            skill_tokens.append(f"SKILL_{skill_id}_b")
        ...
        # Common mistakes (if they have skill_id)
        for i, mistake in enumerate(retrieved_memories.get('mistakes_to_avoid', [])):
            skill_id = mistake.get('skill_id', f'mistake_{i}')
            skill_tokens.append(f"SKILL_{skill_id}_a")
            skill_tokens.append(f"SKILL_{skill_id}_b")
