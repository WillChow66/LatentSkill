...
class SkillsOnlyMemory(BaseMemory):
    ...
    * ``"embedding"`` – the task description is encoded with a
    ...
    def __init__(
        ...
        self.latent_token_mode = latent_token_mode  # Output skill token strings instead of text
        # Must match the k the latent library was baked with (expand_vocab_with_skills),
        # else the prompt silently references only a subset of each skill's latents.
    ...
    def format_for_prompt(self, retrieved_memories: Dict[str, Any]) -> str:
        ...
        If latent_token_mode is True, outputs skill token strings (e.g., SKILL_gen_001_a)
        that the tokenizer will map to pre-initialized latent skill embeddings.
        Otherwise outputs original text skill descriptions.
        """
        # === LATENT TOKEN MODE: output skill token strings ===
        if self.latent_token_mode:
            # Suffix convention MUST mirror expand_vocab_with_skills._suffix_for:
            # a, b, c, ... for j<26 then x{j}. k comes from self.latents_per_skill.
            def _suffixes():
                return [chr(ord('a') + j) if j < 26 else f"x{j}"
                        for j in range(self.latents_per_skill)]
            sfx = _suffixes()
            skill_tokens = []
            for skill in retrieved_memories.get('general_skills', []):
                sid = skill.get('skill_id', '')
                skill_tokens.extend([f"SKILL_{sid}_{s}" for s in sfx])
            for skill in retrieved_memories.get('task_specific_skills', []):
                sid = skill.get('skill_id', '')
                skill_tokens.extend([f"SKILL_{sid}_{s}" for s in sfx])
            for i, mistake in enumerate(retrieved_memories.get('mistakes_to_avoid', [])):
                sid = mistake.get('skill_id', f'mistake_{i}')
                skill_tokens.extend([f"SKILL_{sid}_{s}" for s in sfx])
            return " ".join(skill_tokens) if skill_tokens else ""

        # === ORIGINAL TEXT MODE (unchanged from SkillRL) ===
        sections = []
