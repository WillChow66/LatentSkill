...
class ActorRolloutRefWorker(Worker):
    ...
    def _build_model_optimizer(
        ...
        # === Latent Skill Composer Integration (Plan C) ===
        # Key idea: Load a SEPARATE, non-FSDP, frozen encoder copy for Composer.
        # This avoids all FSDP integration issues (meta tensor, DTensor, lm_head gather).
        # Memory cost: +14GB per GPU (for 7B model in bf16). GH200 120GB has room.
        composer_config = self.config.get("composer", None)
        if composer_config is not None and role == "actor":
            import json
            from src.skill_composer import SkillComposer, setup_model_overrides

            logger.info("[Composer] Initializing (Plan C: separate encoder copy)...")
            device = get_torch_device().current_device()

            # 1) Create SkillComposer (encoder_model=None for now; will set after FSDP)
            #    hidden_size from config avoids meta tensor issue on non-rank-0 workers
            k = composer_config.get("latents_per_skill", 2)
            hidden = actor_model_config.hidden_size
            self._composer = SkillComposer(
                encoder_model=None,  # set after FSDP wrap (loads encoder_copy)
                latents_per_skill=k,
                hidden_size=hidden,
            )

            # 2) Override query_latents (keep fp32 for optimizer precision;
            #    cast to bf16 happens in encode_skills forward as needed)
            #    bf16's ~0.25 precision near norm=33 would otherwise quantize tiny RL updates to zero.
            real_init = (torch.randn(k, hidden) * 0.02).to(torch.float32).to(device)
            self._composer.query_latents = nn.Parameter(real_init)

            # 3) Load pre-trained query_latents if provided
            pretrained_path = composer_config.get("pretrained_query_latents", None)
            if pretrained_path:
                self._composer.load_pretrained(pretrained_path)
                logger.info(f"[Composer] Loaded pre-trained query_latents from {pretrained_path}")

            # 4) Setup skill-to-token mapping
            skills_json = composer_config.get("skills_json_path", None)
            skill_token_map_path = composer_config.get("skill_token_map_path", None)
            if skills_json and skill_token_map_path:
                with open(skill_token_map_path) as f:
                    skill_token_map = json.load(f)
                self._composer.setup_skills(skills_json, self.tokenizer, skill_token_map)

            # 5) Monkey-patch actor_module's embed_tokens (before FSDP wrap!)
            #    SKILL token positions will be replaced by Composer's latent output
            setup_model_overrides(actor_module, self._composer)

            # 6) Freeze ALL base model parameters
            for param in actor_module.parameters():
                param.requires_grad = False
            self._composer.query_latents.requires_grad = True

            n_trainable = self._composer.query_latents.numel()
            n_frozen = sum(p.numel() for p in actor_module.parameters())
            logger.info(f"[Composer] Trainable: {n_trainable:,} params (query_latents)")
            logger.info(f"[Composer] Frozen: {n_frozen:,} params (base model)")

            # Note: encoder_copy is loaded AFTER FSDP wrapping in Plan C (see below)
        else:
            self._composer = None
        # === End Composer Integration ===
        ...
        # === Plan C: Load separate Encoder copy for Composer ===
        # Must be AFTER FSDP wrap (avoids meta tensor issue).
        # This encoder_copy is NOT FSDP-wrapped, so Composer can use it freely.
        # Memory: +14GB per GPU for 7B model in bf16.
        if self._composer is not None and role == "actor":
            logger.info("[Composer] Loading separate encoder copy (non-FSDP, frozen)...")
            encoder_copy = AutoModelForCausalLM.from_pretrained(
                pretrained_model_name_or_path=local_path,
                torch_dtype=torch.bfloat16,
                trust_remote_code=trust_remote_code,
            ).to(get_torch_device().current_device()).eval()
            # Freeze: we only use it as a feature extractor
            for p in encoder_copy.parameters():
                p.requires_grad = False
            # Wire it up to the composer
            self._composer.encoder = encoder_copy
            logger.info("[Composer] Encoder copy loaded and frozen")
            log_gpu_memory_usage("After encoder copy load", logger=logger)
        # === End Plan C Encoder ===
        ...
        if role == "actor" and optim_config is not None:
            ...
            if self._composer is not None:
                # Composer mode: only train query_latents, freeze everything else
                composer_lr = self.config.get("composer", {}).get("lr", 5e-3)
                actor_optimizer = optim.AdamW(
                    [self._composer.query_latents],
                    lr=composer_lr,
                    weight_decay=0.0,  # TokMem uses weight_decay=0 for embeddings
                )
                logger.info(f"[Composer] Optimizer: AdamW, lr={composer_lr}, only query_latents")
            else:
                actor_optimizer = optim.AdamW(
    ...
    def init_model(self):
        ...
        if self._is_actor:
            ...
            if self._composer is not None:
                self.actor._composer = self._composer
    ...
    def save_checkpoint(self, local_path, hdfs_path=None, global_step=0, max_ckpt_to_keep=None):
        ...
        # Save Composer query_latents separately (small, kept across all checkpoints)
        # This survives even when verl's full ckpts get rotated (max_actor_ckpt_to_keep=1)
        if hasattr(self, '_composer') and self._composer is not None and dist.get_rank() == 0:
            ql_dir = os.path.dirname(local_path.rstrip('/'))
            ql_path = os.path.join(ql_dir, f"query_latents_step{global_step}.pt")
            torch.save({
                'query_latents': self._composer.query_latents.detach().cpu(),
                'step': global_step,
                'value_norm': self._composer.query_latents.norm().item(),
            }, ql_path)
            print(f"[Composer] Saved query_latents at step {global_step} to {ql_path}")
