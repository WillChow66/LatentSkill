...
class FSDPVLLMShardingManager(BaseShardingManager):
    ...
    def __enter__(self):
        ...
        if vllm_version in (
            ...
            # === Plan C: Composer encode + TP-aware vLLM embedding sync ===
            if hasattr(self, '_composer') and self._composer is not None:
                with torch.no_grad():
                    latent_map = self._composer.encode_all_skills()

                # TP-aware write to vLLM's VocabParallelEmbedding
                # vLLM shards vocab by token_id; each TP rank has a contiguous range.
                vllm_model = self.model_runner.model
                try:
                    vllm_embed = vllm_model.model.embed_tokens
                except AttributeError:
                    vllm_embed = vllm_model.get_input_embeddings()

                # VocabParallelEmbedding exposes vocab_start/end_index for this rank
                vocab_start = getattr(vllm_embed, 'vocab_start_index', None)
                vocab_end = getattr(vllm_embed, 'vocab_end_index', None)

                if vocab_start is None or vocab_end is None:
                    # Single-GPU vLLM: no sharding
                    vocab_start, vocab_end = 0, vllm_embed.weight.shape[0]

                n_updated = 0
                for token_id, latent_vec in latent_map.items():
                    if vocab_start <= token_id < vocab_end:
                        local_idx = token_id - vocab_start
                        vllm_embed.weight.data[local_idx] = latent_vec.detach().to(
                            vllm_embed.weight.dtype
                        ).to(vllm_embed.weight.device)
                        n_updated += 1

                # Cache detached latents for Actor's old_log_prob / ref_log_prob path
                # (Training update re-encodes with fresh graph in dp_actor.update_policy)
                detached_map = {k: v.detach() for k, v in latent_map.items()}
                self._composer.update_latent_cache(detached_map)
                logger.info(
                    f"[Composer] Synced {n_updated}/{len(latent_map)} SKILL embeddings "
                    f"to vLLM (vocab range [{vocab_start}, {vocab_end}))"
                )
            # === End Plan C ===
