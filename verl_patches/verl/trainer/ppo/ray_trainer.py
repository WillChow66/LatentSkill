...
class RayPPOTrainer:
    ...
    def _validate(self):
        ...
        for test_data in self.val_dataloader:
            ...
            if "env_kwargs" in test_batch.non_tensor_batch.keys():
    ...
    def _save_best_checkpoint(self):
        """Save the best-performing checkpoint to a fixed 'best' directory.
        This complements _save_checkpoint (which is rotated by max_ckpt_to_keep).
        We pass max_ckpt_to_keep=1 so the previous 'best' is overwritten.
        """
        best_folder = os.path.join(self.config.trainer.default_local_dir, "best")
        actor_best_path = os.path.join(best_folder, "actor")
        actor_remote_path = None
        # max_ckpt_to_keep=1 ensures previous best is replaced
        self.actor_rollout_wg.save_checkpoint(actor_best_path, actor_remote_path, self.global_steps, max_ckpt_to_keep=1)
        if self.use_critic:
            critic_best_path = os.path.join(best_folder, "critic")
            self.critic_wg.save_checkpoint(critic_best_path, None, self.global_steps, max_ckpt_to_keep=1)
    ...
    def fit(self):
        ...
        best_val_score = -float('inf')  # track best val/success_rate for best-ckpt saving
        best_val_step = 0
        ...
        for epoch in range(self.config.trainer.total_epochs):
            for batch_dict in self.train_dataloader:
                ...
                if "tools_kwargs" in batch.non_tensor_batch.keys():
                    non_tensor_batch_keys_to_pop.append("tools_kwargs")
                if "env_kwargs" in batch.non_tensor_batch.keys():
                    non_tensor_batch_keys_to_pop.append("env_kwargs")
                gen_batch = batch.pop(
                    batch_keys=batch_keys_to_pop,
                    non_tensor_batch_keys=non_tensor_batch_keys_to_pop,
                ...
                with _timer("step", timing_raw):
                    ...
                    if self.val_reward_fn is not None and self.config.trainer.test_freq > 0 and (is_last_step or self.global_steps % self.config.trainer.test_freq == 0):
                        ...
                        # Track best validation score for best-ckpt saving
                        cur_val_score = val_metrics.get('val/success_rate', -float('inf'))
                        if cur_val_score > best_val_score:
                            best_val_score = cur_val_score
                            best_val_step = self.global_steps
                            print(f"[BestCkpt] New best val/success_rate={best_val_score:.4f} at step {best_val_step}")
                            # Save the best checkpoint (overwrites previous best)
